...
export default function NovoProduto() {
  ...
  const [form, setForm] = useState({
    ...
    destaque: false, novo: true, maisVendido: false, cores: [] as string[],
    imagens: [] as string[],
  });
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    adicionarProduto({
      ...
      imagens: form.imagens, categoria: form.categoria, estoque: parseInt(form.estoque),
      material: form.material || undefined, peso: form.peso || undefined,
      destaque: form.destaque, novo: form.novo, maisVendido: form.maisVendido,
      cores: form.cores.length > 0 ? form.cores : undefined, nota: 0, avaliacoes: [],
    });
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-semibold text-[#1a1a2e]">Novo Produto</h1>
      </div>
      <form onSubmit={handleSubmit} className="max-w-3xl space-y-6">
        <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-4">
          <h2 className="text-[14px] font-semibold text-[#1a1a2e] border-b pb-3">Informações</h2>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-4">
          <h2 className="text-[14px] font-semibold text-[#1a1a2e] border-b pb-3">Precos</h2>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-[12px] font-medium text-[#666] mb-1">Preco (R$) *</label><input type="number" required step="0.01" min="0" value={form.preco} onChange={(e) => setForm({...form, preco: e.target.value})} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-[13px] outline-none focus:border-[#D4A843]" /></div>
            <div><label className="block text-[12px] font-medium text-[#666] mb-1">Preco promocional</label><input type="number" step="0.01" min="0" value={form.precoPromocional} onChange={(e) => setForm({...form, precoPromocional: e.target.value})} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-[13px] outline-none focus:border-[#D4A843]" placeholder="Vazio = sem promo" /></div>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-4">
          <h2 className="text-[14px] font-semibold text-[#1a1a2e] border-b pb-3">Detalhes</h2>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-[12px] font-medium text-[#666] mb-1">Material</label><input type="text" value={form.material} onChange={(e) => setForm({...form, material: e.target.value})} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-[13px] outline-none focus:border-[#D4A843]" placeholder="Prata 925" /></div>
            <div><label className="block text-[12px] font-medium text-[#666] mb-1">Peso</label><input type="text" value={form.peso} onChange={(e) => setForm({...form, peso: e.target.value})} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-[13px] outline-none focus:border-[#D4A843]" placeholder="12g" /></div>
          </div>
          {/* Upload de imagens */}
          <div>
            ...
            <input type="text" placeholder="Ou cole a URL da imagem e pressione Enter..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-[12px] outline-none focus:border-[#D4A843]"
        ...
        <div className="flex gap-3">
          <button type="submit" className="bg-[#1B3A5C] text-white px-8 py-3 text-[13px] font-medium rounded-lg hover:bg-[#243F6B]">Cadastrar Produto</button>
          <button type="button" onClick={() => router.back()} className="border border-gray-200 text-[#666] px-8 py-3 text-[13px] rounded-lg hover:border-gray-300">Cancelar</button>
        </div>
      </form>
    </div>
  );
